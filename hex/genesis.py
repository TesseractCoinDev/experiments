...
def partition():
  ...
  while len(txids) > 1:
      ...
      txids = merklep
  merkleRoot = txids[0]  
  signature = signings.sign_digest(keccak(version.to_bytes(4, "big") + timestamp.encode("utf-8") + nonce.to_bytes(80, "big") + extraNonce.to_bytes(80, "big") + sub_target + merkleRoot))
...
def hex():
  ...
  signature = signings.sign_digest(keccak(version.to_bytes(4, "big") + height.to_bytes(4, "big") + prevHash.encode("utf-8") + timestamp.encode("utf-8") + merkleRoot))
